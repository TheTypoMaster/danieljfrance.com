...
var Admin = {
	...
	init : function(){
		...
		$('select.data,input.data').on('change', this.pageEdited);
	...
	recreateCKEditorFields : function()
	{
        for(instance in CKEDITOR.instances){
            // console.log(CKEDITOR.instances[instance].container.$.setAttribute('contenteditable','true'));
	...
	savePage : function()
	{
		...
		$.ajax({
			...
			{
				// if(DEBUG) console.log(page_content);
				if(data.success == 1){
					Admin.pageSaved();
				}
			}
	...
	addVideoToGallery : function()
	{
		$.ajax({
			url: '/admin/galleries/add-video',
			type: 'post',
			data: {
				"gallery_id" : $('#gallery_id').val(),
				"video_id" : $('#video_to_add').val(),
				"title" : $('#video_title').val(),
				"description" : $('#video_description').val(),
			},
			success: function(data) {
				Admin.addItemToGalleryTable(data.item);
				$('#video_title').val('');
	...
	addItemToGalleryTable : function(item)
	{
		...
		$('#gallery_table').find('tbody').append(row);
	},

	addComponent : function(component)
	{
		var page_content_id = $('.page-content').attr('data-id');
		// console.log(page_content_id);
